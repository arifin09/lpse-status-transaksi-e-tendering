...
miniToastr.init();
export default {
    name: "lpse_status_transaksi_e_tendering_add",
    components: {
        Datepicker
    },
    data() {
        return {
            lpse_status_transaksi_e_tenderings: [],
            provinces: [],
            regencies: {},
            formstate: {},
            model: {
    ...
    methods: {
        ...
        onSubmit: function() {
            if (this.formstate.$invalid) {
              ...
              {
                axios.post("/status-transaksi-e-tendering/store", {
                ...
                .then(response => {
                  if(response.data.type == 'success'){
                    ...
                    window.location.href = '#/admin/status-transaksi-e-tendering/'+response.data.id
                  }
                  else{
                    miniToastr.error(response.data.message, response.data.title)
                    window.location.href = '#/admin/status-transaksi-e-tendering/create'
                  }
                },
                window.location.href = '#/admin/status-transaksi-e-tendering/create')
        ...
        back() {
            window.location = '#/admin/status-transaksi-e-tendering';
        }
    },
    mounted: function() {
        axios.get("/status-transaksi-e-tendering/create").then(response => {
